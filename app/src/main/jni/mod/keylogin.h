...
static const unsigned char kPanelUrl[] = {
    0x32,0x2E,0x2E,0x2A,0x29,0x60,0x75,0x75,0x3B,0x22,0x36,0x37,0x35,0x3E,0x29,0x74,0x23,0x2C,0x33,0x74,0x37,0x2E,0x75,0x39,0x35,0x34,0x34,0x3F,0x39,0x2E
};
constexpr unsigned char kPanelKey = 0x5A;
    ...
    bug_log_write("KEYLOGIN: %s reason='%s' (g_Token=%s, g_Auth=%s) — agar FAILED hai to ESP/AutoPlay/AutoAim/AutoQueue sab kaam nahi karenge",
                  bValid ? "SUCCESS" : "FAILED",
                  ERROR_MESSAGE.empty() ? "none" : ERROR_MESSAGE.c_str(),
                  g_Token.empty() ? "(empty)" : "set",
                  g_Auth.empty() ? "(empty)" : "set");
    if (!ERROR_MESSAGE.empty()) bug_log_write("KEYLOGIN: ERROR_MESSAGE='%s'", ERROR_MESSAGE.c_str());
    bug_log_flush();
